...
[arabic, start=71]
. The evaluator shall verify the implementation of the key establishment schemes  supported by the TOE using the following applicable tests.
...
. There are four Known Answer Tests (KATs) described below to test a basic AES encryption operation (AES-ECB mode). For all KATs, the plaintext and ciphertext values shall be 128-bit blocks. The results from each test may either be obtained by the validator directly or by supplying the inputs to the implementer and receiving the results in response. To determine correctness, the evaluator shall compare the resulting values to those obtained by submitting the same inputs to a known good implementation.
...
[arabic, start=152]
. The evaluators devise an input set consisting of m/8+1 messages, where m is the _rate_ of the hash algorithm. The length of the messages ranges sequentially from 0 to m/8 bytes (or 1 to m/8 bytes, if zero-length is not supported by the hash function), with each message being an integral number of bytes. The message text shall be pseudorandomly generated. The evaluators compute the message digest for each of the messages and ensure that the correct result is produced when the messages are provided to the TSF.
...
*FCS_IPSEC_EXT.1.7*
[arabic, start=322]
. The evaluator shall ensure the TSS identifies the lifetime configuration method used for limiting the IKEv1 Phase 1 SA lifetime and/or the IKEv2 SA lifetime. The evaluator shall verify that the description corresponds to the selection in FCS_IPSEC_EXT.1.5.

*FCS_IPSEC_EXT.1.8*
[arabic, start=323]
. The evaluator shall ensure the TSS identifies the lifetime configuration method used for limiting the IKEv1 Phase 2 SA lifetime and/or the IKEv2 Child SA lifetime. The evaluator shall verify that the description corresponds to the selection in FCS_IPSEC_EXT.1.5.
...
===== Guidance Documentation
[arabic, start=383]
. The evaluator shall examine the guidance documentation to determine that any necessary preparatory steps to ensure authentication data is not revealed for each local login allowed.
