...
.. Test 2 [conditional]: If administrator override is selected, the evaluator shall then change the presented certificate(s) so that validation fails due to a failure to to determine revocation status and show that the certificate is not automatically accepted. The evaluator shall perform the action indicated in the SFR selection observing the TSF resulting in the expected state for the trusted channel (e.g. trusted channel was established).
.. Test 3: While performing testing of invalid DTLS Client Reference Identifiers, expired X.509 certificates, and invalid X.509 trust chains; the evaluator shall ensure the TSF does not present an administrator override option, with the expcetion of failure to to determine revocation status (if selected). Note: This should be a review of behiavor observed while performing other tests.
