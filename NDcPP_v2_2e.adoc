...
If an SSH channel is used for communication between the components, then the TSF shall be validated against the  version of the Functional Package for Secure Shell stated in the "Allowed Packages" list. The SFR requirements in the functional package shall be considered "Feature Dependent" for the allocation of the SFRs for the Distributed TOE.
...
* Requirements for the use of secure communication protocols implemented by allowed packages may be found in the respective package's document.
