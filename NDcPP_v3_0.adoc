...
[cols=",",]
|===
|[SD] |Evaluation Activities for Network Device cPP, Version 4.0e
|===
...
|4.0-draft | 31-Mar-2025 | Incorporated comments received
...
_This requirement is claimed when a DRBG is seeded with entropy from one or more noise source that is outside the TOE boundary. In the case of a network device this would only occur with a vND where the entropy source is provided by the underlying Virtualisation System (VS) platform.  Typically the entropy produced by an environmental noise source is conditioned such that the input length has full entropy and is therefore usable as the seed. However, if this is not the case, it should be noted what the minimum entropy rate of the noise source is so that the TSF can collect a sufficiently large sample of noise data to be conditioned into a seed value._
