...
* Requirements for the use of secure communication protocols are set for allowed protocols in FCS_HTTPS_EXT.1, FCS_IPSEC_EXT.1, the Functional Package for SSH, and the Functional Package for TLS.
...
* Additional optional use of certificate-based protection of signatures can be specified using FPT_TUD_EXT.2, supported by the X.509 certificate processing requirements in the Functional Package for TLS
...
_The option "FFC Schemes using “safe-prime” groups that meet the following: ‘NIST Special Publication 800-56A Revision 3, “Recommendation for Pair-Wise Key Establishment Schemes Using Discrete Logarithm Cryptography” and [selection: groups listed in RFC 3526, groups listed in RFC 7919]." shall be read as 'the TOE performs Key Agreement as specified in SP800-56Ar3', but not necessarily adhering to the protocol restrictions for these groups, as indicated in Appendix D, tables 25 and 26._ _Instead, the use of those methods for particular protocols is in accordance with the SFR for the specific protocols. 
...
Support for X.509 certificates is strictly optional in the context of FPT_ITT.1 (FTP_ITC.1 and FTP_TRP.1/Admin are discussed elsewhere). This is because the use of FPT_ITT.1 itself as a registration channel is optional, so it's further optional to implement that using a mechanism that relies on X.509 validation. 
...
==== Cryptographic Protocols (Extended - FCS_DTLSC_EXT, FCS_DTLSS_EXT

=====  FCS_DTLSC_EXT & FCS_DTLSS_EXT DTLS Protocol

Datagram TLS (DTLS) is not a required component of the NDcPP. If a TOE implements DTLS, a corresponding selection in FTP_ITC.1, FTP_TRP.1/Admin, or FPT_ITT.1 should be made to define what the DTLS protocol is implemented to protect. If a corresponding option to support DTLS has been selected in at least one of the SFRs named above, the corresponding SFRs in the Functional Package for TLS should be added to the ST.

A TOE may act as the client, the server, or both in DTLS sessions. The Functional Package for TLS has been separated into DTLS Client (FCS_DTLSC_EXT) and DTLS Server (FCS_DTLSS_EXT) requirements to allow for these differences. This PP does not mandate support for mutual authentication or any other selectable claim from the Functional Package for TLS.

=====  FCS_TLSC_EXT & FCS_TLSS_EXT DTLS Protocol

TLS is not a required component of the NDcPP. If a TOE implements TLS, a corresponding selection in FTP_ITC.1, FTP_TRP.1/Admin, or FPT_ITT.1 should be made to define what the TLS protocol is implemented to protect. If a corresponding option to support TLS has been selected in at least one of the SFRs named above, the corresponding SFRs in the Functional Package for TLS should be added to the ST.

A TOE may act as the client, the server, or both in TLS sessions. The Functional Package for TLS has been separated into TLS Client (FCS_TLSC_EXT) and TLS Server (FCS_TLSS_EXT) requirements to allow for these differences. This PP does not mandate support for mutual authentication or any other selectable claim from the Functional Package for TLS.
...
Support for X.509 certificates is required if the TSF implements IPsec, TLS, or DTLS communications in FTP_ITC.1 or FTP_TRP.1/Admin (FPT_ITT.1 is a special case covered in Appendix A.3.1.1), or if the TSF implements SSH in a manner where X.509 certificates are used for authentication. X.509 support is also required if the TSF uses X.509 certificates as an integrity mechanism for software updates (i.e. when FPT_TUD_EXT.2 is claimed).

The Functional Package for X.509 defines the capability to assert identities and the capability to verify identities in its mandatory SFR, FIA_XCU_EXT.1. The ST author selects "verify" in cases where the TOE is presented a certificate from an external entity that is using it to validate its identity (e.g. if the TOE is a TLS client, the TOE is a TLS server that requires mutual authentication, the TOE implements IPsec, the TOE uses code signing certificates to validate software/firmware updates). The ST author selects "assert" in cases where the TOE presents a certificate to an external entity (e.g. if the TOE is a TLS server, the TOE is a TLS client where the server requires mutual authentication, the TOE implements IPsec, the TOE implements SSH and uses X.509 to validate the server). 

In all cases, the relevant SFR dependencies in the Functional Package for X.509 are claimed. For certificate verification, this includes FIA_X509_EXT.1 and FIA_X509_EXT.2. For certificate assertion, this includes FIA_XCU_EXT.2 (and FIA_X509_EXT.3 in cases where the TOE's certificate is obtained from an external CA rather than acting as its own CA). 

When certificate validation is claimed for communication with external entities, the following restrictions are applied to the Functional Package for TLS: 
- FIA_X509_EXT.1.1: either an unlimited maximum path length must be selected, or a maximum path length of 3 or greater must be claimed, UNLESS the certificate is used for FPT_ITT.1 communications, in which case a path length of 2 is permissible. 
- FIA_X509_EXT.1.3: at least one of CRL or OCSP must be supported for revocation checking. 
...
==== Cryptographic Protocols (FCS_HTTPS_EXT, FCS_IPSEC_EXT, FCS_NTP_EXT)
...
|FCS_HTTPS_EXT.1 |FCS_TLSC_EXT.1 or FCS_TLSS_EXT.1 |FCS_TLSC_EXT.1 and FCS_TLSS_EXT.1 defined in Functional Package for TLS
|FCS_IPSEC_EXT.1 a|
FCS_CKM.1
