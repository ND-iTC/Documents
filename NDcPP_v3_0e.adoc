...
In all cases except FCS_HTTPS_EXT.1, the relevant SFR dependencies in the Functional Package for X.509 are claimed. For certificate verification, this includes FIA_X509_EXT.1 and FIA_X509_EXT.2. For certificate assertion, this includes FIA_XCU_EXT.2 (and FIA_X509_EXT.3 in cases where the TOE's certificate is obtained from an external CA rather than acting as its own CA). The dependence on FCS_HTTPS_EXT.1 when EST or CMC using HTTPS is claimed is satisfied by FCS_TLSC_EXT.1 from the Functional Package for Transport Layer Security.
...
* Iteration naming: For consistency, use the iteration "/Rev" for SFR claims related to validating X.509 certificates presented by the operational environment, and use "/ITT" for SFR claims related to validating X.509 certificates presented by another component of a distributed TOE. Both the validation of X.509 certificates requested from an external CA and verifies external entities using X.509 certificates should be included in the appropriate FIA_X509_EXT.1 iteration. If boht "assert" and "verify" related behavior are included in an FIA_X509_EXT.1 iteration, the behavior should be disambiguated in the TSS.
...
* Revocation Checking related to validating X.509 certificates presented by the operational environment: At least one of CRL or OCSP (except local OCSP responder) must be supported for revocation checking. If "cached CRL" is selected, the TOE must refresh the CRL cache from a network source without administrator interaction.
...
* Revocation Checking of the X.509 certificate the TOE presents to an external entity: Any method of revocation checking may be selected. This includes the selections of "Administrative notification of revocation."
