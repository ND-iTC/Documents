...
|FAU_GEN_EXT.1 |Security audit generation for distributed TOE component |All
|FAU_STG_EXT.2 |Protected audit data storage |All
|FAU_STG.2 |Protected audit data storage |Feature Dependent
|FAU_STG_EXT.3 |Counting lost audit data |Feature Dependent
|FAU_STG_EXT.4 |Action in case of possible audit data loss |Feature Dependent
|FAU_STG_EXT.5 |Protected local audit data storage for distributed TOEs |Feature Dependent
|FAU_STG_EXT.6 |Protected remote audit data storage for distributed TOEs |Feature Dependent
...
// RW: This will need updating with the used SFR titles (But note that CC 2022 uses lower case!) //
...
|FIA_AFL.1 |Authentication failures |One
...
* Requirements for use of cryptographic schemes are set in FCS_COP.1/DataEncryption, FCS_COP.1/SigGen, FCS_COP.1/CMAC, FCS_COP.1/Hash, and FCS_COP.1/KeyedHash
...
*FAU_STG_EXT.1 Audit data storage location*

*FAU_STG_EXT.1.1 The TSF shall be able to store generated audit data on the TOE itself and transmit the generated audit data to an external IT entity using a trusted channel according to FTP_ITC. In addition, [selection:
...
===== FAU_STG_EXT.2 Protected Audit Event Storage

*FAU_STG_EXT.2 Protected Audit Event Storage*

*FAU_STG_EXT.2.1* The TSF shall be able to transmit the generated audit data to an external IT entity using a trusted channel according to FTP_ITC.1.
...
*FAU_STG_EXT.2.2* See FAU_STG_EXT.1.1


*FAU_STG_EXT.2.3* The TSF shall maintain a [_selection: log file, database, buffer, [assignment:other local logging method]_] of audit records in the event that an interruption of communication with the remote audit server occurs.

*FAU_STG_EXT.2.4* The TSF shall be able to store [_selection: persistent, non-persistent_] audit records locally with a minimum storage size of [_assignment: number of records and/or file/buffer size(s)_].
...
*FAU_STG_EXT.2.5* The TSF shall [_selection: drop new audit data, overwrite previous audit records according to the following rule: [assignment: rule for overwriting previous audit records], [assignment: other action]_] when the local storage space for audit data is full.
...
*FAU_STG_EXT.2.6* The TSF shall provide the following mechanisms for administrative access to locally stored audit records [_selection: none, manual export, ability to view locally_].
...
* _RSA schemes using cryptographic key sizes of [assignment: 2048 bits or greater] that meet the following: FIPS PUB 186-4, “Digital Signature Standard (DSS)”, Appendix B.3;_
...
_The ST author selects “X.509 certificate” when the TOE uses X.509 certificates in a manner compliant with the certificate validation requirements in the Functional Package for X.509. The digital signature algorithm must be one of the algorithms specified in FCS_COP.1/SigGen._

_The ST author selects ‘digital signature’ for all other digital mechanisms (e.g. X.509 certificates that do not meet the certificate validation requirements in the Functional Package for X.509, GPG, raw public key). The digital algorithm must be one of the algorithms specified in FCS_COP.1/SigGen._
...
// Note this is straight from CC 3.1 R5 (not extended) : In CC2022 this has been renumbered to FAU_STG.2

*FAU_STG.2 Protected audit data storage

*FAU_STG.2.1* FAU_STG.1.1 The TSF shall protect the stored audit records in the audit trail from unauthorised deletion.

*FAU_STG.2.2* The TSF shall be able to prevent unauthorised modifications to the stored audit records in the audit trail.

=====  FAU_STG.3 Counting lost audit data
// renumbered from FAU_STG_EXT.2

*FAU_STG_EXT.3 Counting lost audit data*

*FAU_STG_EXT.3.1* The TSF shall provide information about the number of [selection: _dropped, overwritten, [assignment: other information]_] audit records in the case where the local storage has been filled and the TSF takes one of the actions defined in FAU_STG_EXT.1.5.
...
=====  FAU_STG_EXT.4 Action in case of possible audit data loss

*FAU_STG_EXT.4 Action in case of possible audit data loss*

*FAU_STG_EXT.4.1* The TSF shall generate a warning to inform the Administrator before the audit trail exceeds the local audit trail storage capacity.
...
_If FAU_STG_EXT.4 is added to the ST, the ST has to make clear any situations in which audit records might be “invisibly lost”._
...
===== FAU_STG_EXT.5 Protected local audit event storage for distributed TOEs
...
===== FAU_STG_EXT.6 Protected remote audit data storage for distributed TOEs
...
*FAU_STG_EXT.6.1* Each TOE component which does not store security audit data locally shall be able to buffer security audit data locally until it has been transferred to another TOE component that stores or forwards it. All transfer of audit records between TOE components shall use a protected channel according to [selection: _FPT_ITT.1, FTP_ITC.1_].
...
FAU_STG_EXT.1 Audit data storage location requires that the storage location(s) for audit data be specified.

FAU_STG_EXT.2 Protected audit event storage requires the TSF to use a trusted channel implementing a secure protocol.

FAU_STG_EXT.3 Counting lost audit data requires the TSF to provide information about audit records affected when the audit log becomes full.

FAU_STG_EXT.4 Action in case of possible audit data loss requires the TSF to generate a warning before the audit trail exceeds the local storage capacity.

FAU_STG_EXT.5 Protected local audit event storage for distributed TOEs requires the TSF to use a trusted channel to protect audit transfer to another TOE component.

FAU_STG_EXT.6 Protected remote audit event storage for distributed TOEs requires the TSF to use a trusted channel to protect audit transfer to another TOE component.

*Management: FAU_STG_EXT.1, FAU_STG_EXT.2, FAU_STG_EXT.3, FAU_STG_EXT.4, FAU_STG_EXT.5, FAU_STG_EXT.6*
...
*Audit: FAU_STG_EXT.1, FAU_STG_EXT.2, FAU_STG_EXT.3, FAU_STG_EXT.4. FAU_STG_EXT.5, FAU_STG_EXT.6*
...
*FAU_STG_EXT.1.1*
The TSF shall be able to store generated audit data on the TOE itself and transmit the generated audit data to an external IT entity using a trusted channel according to FTP_ITC. In addition, [selection:
...
=====  FAU_STG_EXT.2 Protected audit data storage
...
*FAU_STG_EXT.2.2* The TSF shall maintain a [selection: log file, database, buffer, [assignment:other local logging method]] of audit records in the event that an interruption of communication with the remote audit server occurs.

*FAU_STG_EXT.2.3* The TSF shall be able to store [selection: persistent, non- persistent] audit records locally with a minimum storage size of [assignment: number of records, file/buffer size(s)].

*FAU_STG_EXT.2.4* The TSF shall [selection: drop new audit data, overwrite previous audit records according to the following rule: [assignment: rule for overwriting previous audit records], [assignment: other action]] when the local storage space for audit data is full.

*FAU_STG_EXT.2.5* The TSF shall provide the following mechanisms for administrative access to locally stored audit records [selection: none, manual export, ability to view locally].
...
* FAU_GEN.1 Audit data generation
// Should this be FAU_GEN_EXT.1? //

* FAU_STG_EXT.2 Protected audit data storage


*FAU_STG_EXT.3.1* The TSF shall provide information about the number of [selection: dropped, overwritten, [assignment: other information]] audit records in the case where the local storage has been filled and the TSF takes one of the
...
*FAU_STG_EXT 4.1* The TSF shall generate a warning to inform the Administrator
...
*FAU_STG_EXT.5.1* The TSF of each TOE component which stores security audit data locally shall perform the following actions when the local storage space for audit data is full: [assignment: table of components and for each component its
action chosen according to the following: [selection: drop new audit data, overwrite previous audit records according to the following rule: [assignment: rule for overwriting previous audit records], [assignment: other action]]].

=====  FAU_STG_EXT.6 Protected Remote Audit Event Storage for Distributed TOEs
Hierarchical to: No other components
...
*FAU_STG_EXT.6.1* Each TOE component which does not store security audit data locally shall be able to buffer security audit data locally until it has been transferred to another TOE component that stores or forwards it. All transfer of
audit records between TOE components shall use a protected channel according to [selection: FPT_ITT.1, FTP_ITC.1].
