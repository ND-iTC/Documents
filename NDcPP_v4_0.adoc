...
The PP-Modules that are allowed to specify this cPP as a PP-Module base are specified in the 'Allowed-with' PP-Modules list at https://nd-itc.github.io/AWL/NDcPP_allowed_with_list.html.
...
[[table-7-fcs-cop11-keyedhash]]_Table 7: Allowed choices for FCS_COP.1.1/KeyedHash_
...
_This requirement is claimed when a TOE uses a single internal source of entropy to initialize or reseed a DRBG that is within the TOE boundary. Seeding a DRBG is the same as initializing a DRBG._
