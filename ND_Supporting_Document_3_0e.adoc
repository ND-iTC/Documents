...
*Key Generation for FIPS PUB 186-5 RSA*
...
[arabic, start=123]
. The evaluator shall examine the TSS to determine that it specifies the cryptographic algorithm and key size supported by the TOE for signature generation services.

===== Guidance Documentation

[arabic, start=124]
. The evaluator shall verify that the AGD guidance instructs the administrator how to configure the TOE to use the selected cryptographic algorithm and key size defined in the Security Target supported by the TOE for signature generation services.
