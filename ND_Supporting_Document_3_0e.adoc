...
[arabic, start=396]
. The TLS protocol implementation shall be tested in accordance with testing performed for FCS_TLSC_EXT or FCS_TLSS_EXT requirements from the Functional Package for TLS as necessary (i.e., depending on whether the TOE is an HTTPS client, an HTTPS server, or both).
. If the TOE is an HTTPS client or an HTTPS server utilizing X.509 client authentication, then the certificate validity shall be tested in accordance with testing performed for FIA_X509_EXT.1 from the Functional Package for X.509.
