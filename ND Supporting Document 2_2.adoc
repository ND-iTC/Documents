...
[arabic, start=256]
. The evaluator shall examine the TSS to ensure it describes in what circumstances the check of validity of the certificates takes place (e.g., when the TOE is presented with a certificate during secure channel negotiation), and that the TSS identifies any of the rules for checking extendedKeyUsage fields. If selected, the TSS shall list all supported revocation checking methods, specify in what circumstances revocation checking is performed, and categorically describe what certificates are checked (e.g., all certificates up to a Root CA). The evaluator shall confirm the TSS describes trust store implementation or explicitly states that the trust store is not implemented.
