...
.. Test 5: The evaluator shall cause an interrupt in the connection between the TOE and remote audit server. During interruption, the evaluator shall perform a TSF-mediated action and verify the event is recorded in the audit trail. The evaluator shall re-establish the connection to the audit server and repeat the action performing, confirming it is logged to the remote audit server.
