...
[cols=",",]
|===
|[SD] |Evaluation Activities for Network Device cPP, Version 4.0
|===
...
// The above double of FAU_STG_EXT.1 must be an error.//
...
// See https://github.com/ND-iTC/Documents/issues/352 //
// Also extended definition will need to be changed. //
...
// FTP_ITC.1.1 has been rewritten in CC2022.//
...
==== Security audit data storage (FAU_STG.1 & Extended – FAU_STG_EXT)
...
In all cases except FCS_HTTPS_EXT.1, the relevant SFR dependencies in the Functional Package for X.509 are claimed. For certificate verification, this includes FIA_X509_EXT.1 and FIA_X509_EXT.2. For certificate assertion, this includes FIA_XCU_EXT.2 (and FIA_X509_EXT.3 in cases where the TOE's certificate is obtained from an external CA rather than acting as its own CA). FCS_TLSC_EXT.1 from the Functional Package for Transport Layer Security satisfies the Functional Package for X.509 dependency on FCS_HTTPS_EXT.1.
...
[#_Toc27041932]#Table 13: SFR Dependencies Rationale for Selection-Based SFRs#
