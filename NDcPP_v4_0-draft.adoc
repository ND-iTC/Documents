...
As defined by the references [CC1], [CC2], [CC-SFRC], [CC3] and [CCE], this cPP:
...
The supporting document defines the evaluation methods and activities for this cPP and has been developed in accordance [CC4]The supporting document can be found at https://github.com/ND-iTC/Documents.
...
The individual security functional requirements are specified in the sections below. SFRs in this section the mandatory SFRs that any conformant TOE must meet are presented. Based on selections made in these SFRs it will also be necessary to include some of the selection-based SFRs in Appendix B. Additional optional SFRs may also be adopted from those listed in Appendix A.
...
==== Security audit generation (FAU_GEN)
...
[loweralpha]
. Date and time of the audit event, type of event, subject identity +++<del>+++(if applicable)+++</del>+++, and the outcome (success or failure) of the event; and
...
// commentCC2022 change: FAU_STG.1.4 -> FAU_STG.2.5 and FAU_STG.1 is "new" to specify the location where the audit data is stored. 
...
*FTP_TRP.1.1/Admin* The TSF shall *be capable of using [selection: _IPsec, SSH as defined in the Functional Package for SSH as defined in the Functional Package for SSH, TLS as defined in the Functional Package for TLS, DTLS as defined in the Functional Package for TLS, HTTPS_] to* provide a communication path between itself and *authorised* +++<u>+++remote+++</u>+++ *Administrators* +++<del>+++users+++</del>+++ that is logically distinct from other communication paths and provides assured identification of its end points and protection of the communicated data from +++<u>+++disclosure+++</u>+++ *and provides detection of modification of the channel data.*
