...
|FCS_CKM.1/SKG |Cryptographic Key Generation - Symmetric Key |All
...
* Requirements for key generation and key agreement are set in FCS_CKM.1/AKG, FCS_CKM.1/SKG, and FCS_CKM_EXT.7 
...
*FCS_CKM.1.1/SKG Cryptographic Key Generation - Symmetric Key*
...
The following table provides the allowed choices for completion of the selection operations of FCS_CKM.1.1/SKG.

[cols=",,,",options="header",]
|===
|*Identifier* |*Cryptographic Key Generation Algorithm* |*Cryptographic Key Sizes* |*List of Standards*
|RSK|Direct Generation from a Random Bit Generator as specified in FCS_RBG.1|[selection: _256, 384, 512_] bits |NIST SP 800-133 Revision 2 (Section 6.1)[Direct generation of symmetric keys]
|===

[#_Ref237676489]_Table #: Allowed choices for FCS_CKM.1.1/SKG_
...
_Key transport schemes refer to cases in which one party has a key to share with another party. Key encapsulation is used when ML-KEM is used as the method of key establishment. Key wrapping and encrypted channels are used in support of wireless LAN communications._  

_If “key encapsulation” is selected, FCS_COP.1/KeyEncap from Appendix B must be claimed, which specifies the relevant list of standards._

_If “key wrapping” is selected, FCS_COP.1/KeyWrap from Appendix B must be claimed, which specifies the relevant list of standards._
